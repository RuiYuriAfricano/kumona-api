/*
  Warnings:

  - You are about to drop the column `role` on the `User` table. All the data in the column will be lost.

*/
-- AlterTable
ALTER TABLE "User" DROP COLUMN "role",
ADD COLUMN     "photo" TEXT NOT NULL DEFAULT 'data:image/jpeg;base64,';

-- CreateTable
CREATE TABLE "EyeImage" (
    "id" SERIAL NOT NULL,
    "image" TEXT NOT NULL,
    "target" TEXT NOT NULL,
    "userId" INTEGER NOT NULL,
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT "EyeImage_pkey" PRIMARY KEY ("id")
);

-- AddForeignKey
ALTER TABLE "EyeImage" ADD CONSTRAINT "EyeImage_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
